...
void work(proc_t *process) {
    int count = process->self_id*5;

    if (process->io->mutexl)
        request_cs((void*)process);
    
    for (int i = 1; i <= count; i++) {
        size_t slen = snprintf(NULL, 0, log_loop_operation_fmt, process->self_id, i, count);
        char *str = (char*)alloca(slen+1);
        snprintf(str, slen+1, log_loop_operation_fmt, process->self_id, i, count);
        print(str);
    }
